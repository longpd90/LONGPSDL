...
+ (void)setIconForButton:(UIButton *)button icon:(NSString *)icon fontSize:(NSUInteger)size state:(UIControlState)state
{
	if (![button respondsToSelector:@selector(setAttributedTitle:forState:)]) return;
	
	UIFont *basicFont = button.titleLabel.font;
	UIFont *iconFont = [UIFont fontWithName:@"FontAwesome" size:size];
	NSMutableAttributedString *iconString = [[NSMutableAttributedString alloc] initWithString:icon
																				   attributes:@{NSFontAttributeName: iconFont}];
	
	NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"  %@", [button titleForState:state]]
																			   attributes:@{NSFontAttributeName: basicFont}];
	[iconString appendAttributedString:string];
	[button setAttributedTitle:iconString forState:state];
}

+ (void)setIconForButton:(UIButton *)button icon:(NSString *)icon state:(UIControlState)state
{
	[FontAwesomeKit setIconForButton:button icon:icon fontSize:button.titleLabel.font.pointSize state:state];
}

+ (void)setIconForLabel:(UILabel *)label icon:(NSString *)icon fontSize:(NSUInteger)size
{
	if (![label respondsToSelector:@selector(setAttributedText:)]) return;
	
	UIFont *basicFont = label.font;
	UIFont *iconFont = [UIFont fontWithName:@"FontAwesome" size:size];
	NSMutableAttributedString *iconString = [[NSMutableAttributedString alloc] initWithString:icon
																				   attributes:@{NSFontAttributeName: iconFont}];
	...
	NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"  %@", label.text]
																			   attributes:@{NSFontAttributeName: basicFont}];
	[iconString appendAttributedString:string];
	label.attributedText = iconString;
}

+ (void)setIconForLabel:(UILabel *)label icon:(NSString *)icon
{
	[FontAwesomeKit setIconForLabel:label icon:icon fontSize:label.font.pointSize];
}
