...
@interface UIImage ()
- (CGContextRef) newARGBBitmapContextFromImage:(CGImageRef) imageRef;
...
- (CGContextRef) newCGContextFromUIImage: (UIImage *) image {
...
- (UIImage *) applyBlend:(UIImage *) image Callback: (void (^)(unsigned char *, unsigned char *)) fn{
    int width0 = self.size.width;
    int height0 = self.size.height;
    int width1 = image.size.width;
    int height1 = image.size.height;
    size_t step = 4;
    if (width0 != width1 || height0 != height1) {
        return NULL;
    }
    
    CGContextRef contex0 = [self newCGContextFromUIImage:self];
    CGContextRef contex1 = [self newCGContextFromUIImage:image];
    ...
    NSInteger length = height0 * width0 * step;
...
- (UIImage *) applyCurve:(Curve)curve {
    ...
    CGContextRef contex = [self newCGContextFromUIImage:self];
    unsigned char * data = (unsigned char *) CGBitmapContextGetData(contex);
    
    NSInteger length = height * width * step;
    for (int i = 0; i < length; i += step) {
...
unsigned char calcOverlay1(unsigned char a, unsigned char b) {
...
- (UIImage *) overlay: (UIImage *) image ratio: (double) ratio {
    ...
    return [self applyBlend: image Callback:^ (unsigned char * data0, unsigned char * data1) {
        for (int i = 1; i < 4; i++) {
            data0[i] = calcBlend(calcOverlay1(data0[i], data1[i]), data0[i], ratio);
        }
    }];
}
...
- (UIImage *) overlay: (UIImage *) image ratio: (double) ratio channel:(int)channel {
    return [self applyBlend: image Callback:^ (unsigned char * data0, unsigned char * data1) {
        data0[channel] = calcBlend(calcOverlay1(data0[channel], data1[channel]), data0[channel], ratio);
    }];
}
...
- (id) applyFiltrrByStep:(int) step ShiftIn:(DataField)shiftIn ShiftOut:(DataField)shiftOut Callback:(RGBA (^)(int r, int g, int b, int a))fn {
    ...
    CGContextRef cgctx = [self newARGBBitmapContextFromImage:self.CGImage];
...
-(unsigned char*) convolveRaw:(NSArray *) kernel InData:(unsigned char *)inData OuData:(unsigned char*)outData Height:(uint)_height Width:(uint)_width {
    ...
    NSInteger kw = [(NSArray *)[kernel objectAtIndex:0] count]/2;
    
    NSInteger i = 0, j = 0, n = 0, m = 0;
    
    for (i = 0; i < _height; i++) {
        for (j = 0; j < _width; j++) {
            NSInteger outIndex = (i*_width*4) + (j*4);
                ...
                for (m = -kw; m <= kw; m++) {
                    if (i+n >= 0 && i + n <_height) {
                        if (j + m  >= 0 && j + m  < _width) {
                            ...
                            NSInteger inIndex = ((i+n)*_width*4) +((j+m)*4);
                            r += inData[inIndex] * f;
...
-(id) applyBlend:(UIImage *)topImage CallBack: (RGBA (^)(RGBA top, RGBA bottom))fn {
    CGContextRef topCGctx = [topImage newARGBBitmapContextFromImage:topImage.CGImage];
    unsigned char* blendData = CGBitmapContextGetData (topCGctx);
    
    CGContextRef bottomCGctx = [self newARGBBitmapContextFromImage:self.CGImage];
    ...
    if(blendData) free(blendData);
		CGContextRelease(bottomCGctx);
		CGContextRelease(topCGctx);
    return newImage;
